...
| `pval_exact()` | Exact p-value by branch pruning with bounds from a dynamic program, without enumerating the sample space. Feasible for a moderate number of categories and trials. |
...
- **The statistic matters.** For the log-likelihood ratio (`"llr"`) and the
  probability mass function (`"pmf"`) the series typically converges in a few hundred
  to a few thousand terms. For Pearson's $\mathcal{X}^2$ (`"chi2"`), and often for the
