...
print(err_p)
stopifnot(grepl("must sum to 1", err_p))
fit_resc <- pval_exact(x_small, c(3, 3, 4), rescale_p = TRUE)
stopifnot(isTRUE(all.equal(fit_resc$pval, fit_ex$pval)), isTRUE(all.equal(fit_resc$p, p_small)))
